...
**Basis:** the owner's prior feasibility analysis (private notes). Do not re-derive it.
...
2. Compose stack in the server's existing stack layout (see the owner's infra notes for conventions): service `chart-manager`, `<nas>/clone-hero:/library`, named volume for `/data`. **First deploy mounts the library `:ro`** — flip to `rw` only after Phase 7 read-only checks pass.
